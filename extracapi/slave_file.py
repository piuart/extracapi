...
#-----------------------------------------------------------------------
    ...
    sleep(randint(4,6))
    print('Ya dejamos el 2 Sleep atrás')

    #-----------Marcamos el tipo de análisi-------------------------------

    choisces_3 = driver.find_element(By.XPATH, '//input[@id ="choices-3"]')
    choisces_3.click()
    
    #Nota: quería hacer de esto algo un poco mas automático... hacer un bugle para recorrer los choisces 
    # y así marcarlos todos, ofreciendo incluso mejoras en el código 
    # anterior, si se podía claro, quería probarlos. 
    ...
    #importante:
    #Queria detectar un evento, para detectar la finalización del análisis, pero no se si era posible dado que la app esta fuera de linea

    #---------------------------------------------------------------------------
    #Si todo esta como la anterior esto después de el análisis recuperamos el reporte
    #código:
    ...
    #app de la url, pero va casi igual, al obtener el análisi identificamos el id de el test
...
#recuperar los valores... pero debido al percance explico mi solución a través como:

#Como la principal dificultad era el usuario , decidí que registrarse 
#siempre era la mejor opción, borrando al final de cada análisis que hiciéramos y así reutilizar las veces que quiera raven/raven.

#Por otro lado... en la vista dentro del proyecto "Test File" tenia la 
#opción de colocar la ruta del archivo, pero vi un poco más cómodo para el usuario 
#arrastrar o copiar todo lols archivos que deseara analizar a una carpeta dentro del proyecto 
#-> files para que dentro de la función slaveFile, detecte la ruta del archivo y  pueda proceder a su análisis.
